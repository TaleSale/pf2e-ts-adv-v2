 ...
 */
(() => {
    // Теперь эти переменные локальные и не вызовут ошибки
    ...
    Hooks.on('renderSpellSheetPF2e', async (app, html, data) => {
        ...
        if (isInnate && isAtWillOrConstant) {
            ...
            if (value !== 99 || max !== 99) {
                console.log(`PF2e Spell Mod: Устанавливаю 99/99 использований для "${item.name}"`);
                await item.update({
