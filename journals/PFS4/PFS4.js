...
class PFS4JournalSheet extends JournalSheet {
  getData(options) {
  ...
  async activateListeners(html) {
    ...
    if (navigationElement) {
      navigationElement.style.background = "url('modules/pf2e-ts-adv/journals/PFS4/back.webp') no-repeat center center";
    ...
    try {
      const response = await fetch('modules/pf2e-ts-adv/journals/PFS4/keywords.json');
...
Hooks.once('init', () => {
  DocumentSheetConfig.registerSheet(JournalEntry, 'PFS4', PFS4JournalSheet, {
    label: "Тема \"PFS4\"",
    makeDefault: false
  });
});
