...
class PFS2JournalSheet extends JournalSheet {
  getData(options) {
  ...
  async activateListeners(html) {
    ...
    if (navigationElement) {
      navigationElement.style.background = "url('modules/pf2e-ts-adv/journals/PFS2/back.webp') no-repeat center center";
    ...
    try {
      const response = await fetch('modules/pf2e-ts-adv/journals/PFS2/keywords.json');
...
Hooks.once('init', () => {
  DocumentSheetConfig.registerSheet(JournalEntry, 'PFS2', PFS2JournalSheet, {
    label: "Тема \"PFS2\"",
    makeDefault: false
  });
});
