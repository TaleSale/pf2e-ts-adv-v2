...
class PFS5JournalSheet extends JournalSheet {

  static get defaultOptions() {
    const options = super.defaultOptions;
    options.classes.push("PFS5");
    return options;
  }
  ...
  async activateListeners(html) {
    ...
    if (navigationElement) {
      // Рекомендуется перенести эти стили в PFS5.css
      navigationElement.style.background = "#063654";
      navigationElement.style.backgroundSize = "cover";
      navigationElement.style.width = '300px';  // установим ширину навигационной части
    }

    if (contentElement) {
      // Рекомендуется перенести этот стиль в PFS5.css
...
Hooks.once('init', () => {
  DocumentSheetConfig.registerSheet(JournalEntry, 'PFS5', PFS5JournalSheet, {
    label: "Тема \"PFS5\"",
    makeDefault: false
  });
});
