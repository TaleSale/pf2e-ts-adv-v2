...
class RotRJournalSheet extends JournalSheet {
  getData(options) {
  ...
  async activateListeners(html) {
    ...
    if (navigationElement) {
      navigationElement.style.background = "url('modules/pf2e-ts-adv-v2/journals/RotR/back.webp') no-repeat center center";
      navigationElement.style.backgroundSize = "cover";
      navigationElement.style.width = '300px';  // установим ширину навигационной части
      navigationElement.style.color = '#4f1adc';
    }
    ...
    try {
      const response = await fetch('modules/pf2e-ts-adv-v2/journals/RotR/keywords.json');
...
Hooks.once('init', () => {
  DocumentSheetConfig.registerSheet(JournalEntry, 'RotR', RotRJournalSheet, {
    label: "Тема \"Месть Рунных Лордов\"",
    makeDefault: false
  });
});
