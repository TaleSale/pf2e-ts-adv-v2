...
class enmitycycleJournalSheet extends JournalSheet {
  getData(options) {
  ...
  static get defaultOptions() {
    ...
    options.classes.push("enmitycycle");
    return options;
  }
  ...
  async activateListeners(html) {
    ...
    if (navigationElement) {
      navigationElement.style.background = "url('modules/pf2e-ts-adv-v2/journals/enmitycycle/back.webp') no-repeat center center";
    ...
    try {
      const response = await fetch('!!!modules/pf2e-ts-adv/enmitycycle/keywords.json');
...
Hooks.once('init', () => {
  DocumentSheetConfig.registerSheet(JournalEntry, 'enmitycycle', enmitycycleJournalSheet, {
    label: "Тема \"Круговорот Вражды\"",
    makeDefault: false
  });
});
