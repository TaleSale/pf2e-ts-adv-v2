...
class SFJournalSheet extends JournalSheet {
  getData(options) {
  ...
  async activateListeners(html) {
    ...
    if (navigationElement) {
      navigationElement.style.background = "url('modules/pf2e-ts-adv/journals/SF/back.webp') no-repeat center center";
    ...
    try {
      const response = await fetch('modules/pf2e-ts-adv/journals/SF/keywords.json');
...
Hooks.once('init', () => {
  DocumentSheetConfig.registerSheet(JournalEntry, 'SF', SFJournalSheet, {
    label: "Тема \"Украденная Судьба\"",
    makeDefault: false
  });
});
